...
El generar compose-sh funciona mediante los mismos comandos que los indicados por la catedra. La cantidad de clientes es parametrizable. A fines de practicidad terminé haciendo que server conozca la cantidad de clients que se levantas. Esto en un ambiente no tan simulado no se podría hacer.

Notar que en los volumenes de los containers, se inyectan tanto el config como el .csv de los datasets.
Los mismos .csv los descomprimí antes de la ejecución.

El bash al ejecutarse genera un compose nuevo con soporte para python, go e incluya los containers que instancio.
...
Para limpiar archivos corruptos:
...
despues en wsl cd /mnt/../tp0-tests,  source venv/bin/activate, REPO_PATH=/../tp0-base make test
...
ahora mismo en python tengo un server single threaded, osea que tiene que cerrar una conexión para poder abrir otra. Ahora me piden que pueda abrir y procesar conexiones usando multi threading. Mi idea era tener un pool de 6 threads y que a medida que un client se conecta que los use, si estan todos ocupados, que el client reintente conectarse en 10 segundos.

Si bien fue aconsejado en el foro el usar la librería de multiprocessing para llevar esta idea, realmente no termine de familiarizarme con el concepto de pickle y que el manager interno funcione acorde.

Termine haciendo una implementación con Threading y Condition donde por cada conexion se paraleliza en un thread, y que para evitar un busy wait se deje dormido el hilo con el condition y un flag. No me pareció lo mas escalable a muchos clients. Pero me termino siendo lo mas práctico
